...
const CommentsSection = ({
  ...
  [getAccessTokenSilently, loginWithPopup, loginWithRedirect]);
  ...
  const fetchComments = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API_URL}/api/posts/${expandedPost._id}/comments`);
  ...
  const submitComment = useCallback(async () => {
    ...
    try {
      ...
      alert(err.response?.data?.message || err.message || 'An error occurred while submitting the comment.');
    } finally {
      setSubmitting(false);
    }
  }, [commentText, expandedPost, getToken]);
      ...
      {loadingComments ? (
        ...
        <div className="space-y-4">
          {comments.map((comment) => (
            <div key={comment._id} className="bg-white border border-slate-200 rounded-2xl p-4">
              <div className="flex items-center gap-3 mb-3">
                <img
                  src={
                    comment.avatar ||
                    `https://ui-avatars.com/api/?name=${encodeURIComponent(comment.fullName || 'User')}`
                  }
                  alt=""
